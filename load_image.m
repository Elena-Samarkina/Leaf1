...
if ~isequal(file, 0)
    ...
    %sharpen image
    I1 = imsharpen(I1,'Radius',5,'Amount',0.8,'Threshold', 0.7);
    % ---------------------
    ...
    print('Udistort  I1')
    global I;
    % I = undistortImage(I1, cameraParams);
    I=I1;
    imshow(I);
    print('Show I')
end
...
%         I1 = fliplr(I1);
    print('Read to I1')
     %sharpen image
    I1 = imsharpen(I1,'Radius',5,'Amount',0.8,'Threshold', 0.7);
    imshow(I1), title('Original sharpen image','FontSize',16);;
    print('Show I1')
    % ---------------------
    load('cameraParams.mat','cameraParams')
    % originalImage = imread(imageFileNames{1});
    ...
    global I;
    I = undistortImage(I1, cameraParams);
    imshow(I), title('Undistort original image','FontSize',16);
...
%      s = fliplr(s);
    I3 = imadjust(s,[0.03 0.997],[]);     

 invImg = ~im2bw(I3, 0.3);
%   invImg = fliplr(invImg);
...
for q=1:L.NumObjects
    ...
    if  rezz>bou_max-bou_max*0.95
      ...
      % angle =atan((Xtwopoint- Xonepoint+1)/(Ytwopoint-Yonepoint+1));
       angle =atan2((Xtwopoint- Xonepoint+1),(Ytwopoint-Yonepoint+1));
        X = grain;
        save('X','X');
       % пересчет угла доворота от центра листа
       Y = imrotate(X, -angle*180.0/pi, 'loose', 'bicubic');
...
% распознаем этот повернутый лист как отдельный оъект
 ...
 list2 = imopen(f1,se1);
 L12 = bwconncomp(list,8);
 L12.NumObjects;
 
 stats1 = regionprops(L1, 'PixelList');
%  stats2 = regionprops(L12, 'PixelList');
...
l1 = line([Xonepoint1 Xtwopoint1],[Yonepoint1 Ytwopoint1 ],[1 1],'Marker','.','LineStyle','-');
 
% вычисление величины корректировки середины площадки вершины листа,
% так как вершина листа и его основание не один пиксель, а больше 
 stats2 = regionprops(L1, 'all');
 Xonepoint1_korr=stats2(1).Extrema;
 korr_top=Xonepoint1_korr(2,1)-Xonepoint1_korr(1,1);
 korr_but=Xonepoint1_korr(5,1)-Xonepoint1_korr(6,1);
 korr_sr= ceil((korr_top+korr_but)/4);
...
% Проверка нетипичных листьев
 ...
 if flass_q<0.55 && flass_q>0
    ...
    title({['Асимметрия листа',num2str(flass_q),'  ',num2str(flass_q2)];['Площадь левая',num2str(area_left)];['правая',num2str(area_right)];['полная',num2str(area)]});
...
%     xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',flass2,'Эталон зеркало1','E2'); 
%     xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',arealeft,'Эталон зеркало1','E3');
%     xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',arearight,'Эталон зеркало1','E4');
 q1=q1+1;
%  end
%  
%  %%%%%      end
% % % Асимметрия зеркального отображения
%  
% 
% 
% % % распознаем зеркальный повернутый лист как отдельный оъект
% %  subplot(2,2,2), imshow(list3),title('Зеркальный лист');    
%  se11 = strel('disk',8);
%  list_zer = imopen(list3,se11);
%  L1_zer = bwconncomp(list_zer,8);
%  L1_zer.NumObjects;
% %  
% %  list2 = imopen(f1,se1);
% %  L12 = bwconncomp(list,8);
% %  L12.NumObjects;
% subplot(2,2,2),imshow(list3),title('Зеркальный лист'); 
%  stats1_zer = regionprops(L1_zer, 'PixelList');
% 
% %  получение параметров  повернутого листа
%   list_PixelList1_zer = [stats1_zer(1).PixelList];
% [min_PixelList1_zer, idx] = min(list_PixelList1_zer);
% [max_PixelList1_zer, idx1] = max(list_PixelList1_zer);
% Yonepoint1 = min_PixelList1_zer(2);
% Ytwopoint1 = max_PixelList1_zer(2);
%  Xonepoint1 = list_PixelList1_zer(idx(2),1);
%  Xtwopoint1 = list_PixelList1_zer(idx1(2),1);
% l1 = line([Xonepoint1 Xtwopoint1],[Yonepoint1 Ytwopoint1 ],[1 1],'Marker','.','LineStyle','-');
% %  
% % % % вычисление величины корректировки середины площадки вершины листа,
% % % % так как вершина листа и его основание не один пиксель, а больше 
...
%  area_right(q)=area_right;
% % % %   получение асимметрии одного листа
% flass_q=ass/(Ytwopoint1-Yonepoint1);
%  flass_q2=ass2/(Ytwopoint1-Yonepoint1);
% 
% area=area_left+area_right;
% 
% imbalance_left=area_left/area;
...
%      arealeft(q)=area_left;
%      arearight(q)=area_right;
%   subplot(2,2,4),imshow(list3),title({['Асимметрия зеркального листа',num2str(flass_q),'  ',num2str(flass_q2)];['Площадь левая',num2str(area_left)];['правая',num2str(area_right)];['полная',num2str(area)]});
%   end
 
    end
%  xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',flass,'Эталон зеркало1','E1'); 
%   xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',flass2,'Эталон зеркало1','E2'); 
%  xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',arealeft,'Эталон зеркало1','E3');
%  xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия_погрешность.xls',arearight,'Эталон зеркало1','E4');
% 
...
% % xlswrite('C:\Users\Elena\Documents\MATLAB\Асимметрия.xls',d,'Асимметрия','E1:F16');
    flass=sum(flass)/q1;
 figure, imshow(f),title(['Асимметрия',num2str(flass)]);
xlswrite(filename,flass,'суммарная','E5');
% 
